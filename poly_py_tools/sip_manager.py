...
if args['generate']:
    ...
    if not os.path.exists(target_sip_path):
        if not args['--force']:
            print("{} does not exist. Use --force to create it, or update the asterisk path in the settings.".format(target_sip_path))
            exit(1)
        else:
            f = open(target_sip_path, 'w')
    ...
    if os.getegid() != 0 and not os.access(configs['paths']['asterisk'], os.W_OK):
        print("I don't have write access to {}. Either check permissions, or run this as root. Cannot continue".format(configs['paths']['asterisk']))
        sys.exit(1)
